...
class ELModel(tf.keras.Model):
    ...
    def call(self, input):
        """Run the model."""
        nf1, nf3, nf4,top, nf3_neg,radius = input
        ...
        loss = loss1 + loss3 + loss4 + loss_top + loss_nf3_neg  - loss7
        return loss
...
class Generator(object):
    ...
    def next(self):
        if self.start < self.steps:
            ...
            self.start += 1
            return ([nf1,nf3, nf4, top, nf3_neg,radius], labels)
        else:
            self.reset()
...
#         nf_chain = Input(shape=(3,), dtype=np.int32)
        ...
        out = el_model([nf1,nf3, nf4,top, nf3_neg,radius])
        model = tf.keras.Model(inputs=[nf1,nf3, nf4,top, nf3_neg,radius], outputs=out)
...
for d in embed_dims:
    ...
    for m in margins:
        ...
        out_classes_file = f'NCI_results/EmEL/NCI_{embedding_size}_{margin}_{epochs}_cls.pkl'
        out_relations_file = f'NCI_results/EmEL/NCI_{embedding_size}_{margin}_{epochs}_rel.pkl'
        loss_history_file= f'NCI_results/EmEL/NCI_lossHis_{embedding_size}_{margin}_{epochs}.csv'
