...
total_sub_cls=[]
train_file = "FMA_data/FMA_train.txt"
va_file = "FMA_data/FMA_valid.txt"
test_file = "FMA_data/FMA_test.txt"
...
class ELModel(tf.keras.Model):
    ...
    def call(self, input):
        """Run the model."""
        nf1, nf3, nf4, top, nf3_neg,nf_inclusion,radius = input
...
#         loss6 = self.chain_loss(nf_chain)
        ...
        loss = loss1 + loss3 + loss4 + loss_top + loss_nf3_neg + loss5 - loss7
        return loss
...
#             nf_chain = self.data['nf_chain'][nf_chain_index]
            ...
            self.start += 1
            return ([nf1, nf3, nf4, top, nf3_neg,nf_inclusion,radius], labels)
        else:
            self.reset()
...
#         nf_chain = Input(shape=(3,), dtype=np.int32)
        ...
        out = el_model([nf1, nf3, nf4, top, nf3_neg, nf_inclusion,radius])
        model = tf.keras.Model(inputs=[nf1, nf3, nf4, top, nf3_neg,nf_inclusion,radius], outputs=out)
...
gdata_file="FMA_data/fma_norm_mod.owl"
train_data_model, classes, relations = load_data2(gdata_file)


# In[26]:


valid_data_file="FMA_data/FMA_valid.txt"
...
for d in embed_dims:
    ...
    for m in margins:
        ...
        out_classes_file = f'FMA_results/EmEL/FMA_{embedding_size}_{margin}_{epochs}_cls.pkl'
        out_relations_file = f'FMA_results/EmEL/FMA_{embedding_size}_{margin}_{epochs}_rel.pkl'
        loss_history_file= f'FMA_results/EmEL/FMA_lossHis_{embedding_size}_{margin}_{epochs}.csv'
