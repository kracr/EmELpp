...
def load_data(filename):
    classes = {}
    relations = {}
    data = {'nf1': [], 'nf2': [], 'nf3': [], 'nf4': [],'nf_inclusion':[],'nf_chain':[]}
...
class ELModel(tf.keras.Model):
    ...
    def call(self, input):
        """Run the model."""
        nf1, nf2, nf3, nf4,top, nf3_neg,nf_inclusion,nf_chain,radius = input
...
#             dis = self.data['disjoint'][dis_index]
            ...
            self.start += 1
            return ([nf1, nf2, nf3, nf4, top, nf3_neg,nf_inclusion,nf_chain,radius], labels)
        else:
            self.reset()
...
for d in embed_dims:
    ...
    for m in margins:
        ...
        out_classes_file = f'SNOMED_results/EmELX/SNOMED_{embedding_size}_{margin}_{epochs}_cls.pkl'
        out_relations_file = f'SNOMED_results/EmELX/SNOMED_{embedding_size}_{margin}_{epochs}_rel.pkl'
        loss_history_file= f'SNOMED_results/EmELX/SNOMED_lossHis_{embedding_size}_{margin}_{epochs}.csv'
