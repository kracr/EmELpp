...
class ELModel(tf.keras.Model):
    ...
    def call(self, input):
        """Run the model."""
        nf1, nf2, nf3, nf4,top, nf3_neg = input
...
#             nf_chain = self.data['nf_chain'][nf_chain_index]
            ...
            self.start += 1
            return ([nf1, nf2, nf3, nf4,top, nf3_neg], labels)
        else:
            self.reset()
...
#         nf_chain = Input(shape=(3,), dtype=np.int32)
        ...
        out = el_model([nf1, nf2, nf3, nf4,top, nf3_neg])
        model = tf.keras.Model(inputs=[nf1, nf2, nf3, nf4,top, nf3_neg], outputs=out)
...
for d in embed_dims:
    ...
    for m in margins:
        ...
        out_classes_file = f'SNOMED_results/EL/SNOMED_{embedding_size}_{margin}_{epochs}_cls.pkl'
        out_relations_file = f'SNOMED_results/EL/SNOMED_{embedding_size}_{margin}_{epochs}_rel.pkl'
        loss_history_file= f'SNOMED_results/EL/SNOMED_lossHis_{embedding_size}_{margin}_{epochs}.csv'
